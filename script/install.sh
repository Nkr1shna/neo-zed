...
main() {
    ...
    if [ "$(command -v zed)" = "$HOME/.local/bin/zed" ]; then
        echo "NeoZed has been installed. Run with 'zed'"
    else
        echo "To run NeoZed from your terminal, you must add ~/.local/bin to your PATH"
        echo "Run:"
        ...
        echo "To run NeoZed now, '~/.local/bin/zed'"
    fi
}
...
linux() {
    if [ -n "${ZED_BUNDLE_PATH:-}" ]; then
        ...
        echo "Downloading NeoZed version: $ZED_VERSION"
