...
#[cfg(test)]
use agent_ui::attach_workspace_controller;
use agent_ui::thread_metadata_store::{SidebarThreadMetadataStore, ThreadMetadata};
use agent_ui::threads_archive_view::{
    ThreadsArchiveView, ThreadsArchiveViewEvent, format_history_entry_timestamp,
};
use agent_ui::{
    Agent, AgentThreadSummary, AiWorkspace, NewThread, RemoveSelectedThread,
    agent_connection_store, load_agent_thread_in_center, thread_store, thread_summaries,
...
impl Sidebar {
    ...
    fn subscribe_to_workspace(
        ...
        cx.subscribe_in(
            ...
            |this, workspace, event: &workspace::Event, _window, cx| {
                if let workspace::Event::AiSurfaceChanged = event {
                    ...
                    this.ai_surface_visible = AiWorkspace::is_visible(workspace, cx);
                    this.observe_draft_editor(cx);
                    this.update_entries(cx);
                }
            },
        ...
        if workspace_controller(&workspace.read(cx), cx).is_some() {
            ...
            self.ai_surface_visible = AiWorkspace::is_visible(workspace, cx);
    ...
    fn observe_draft_editor(&mut self, cx: &mut Context<Self>) {
        self._draft_observation = self
            ...
            .and_then(|mw| {
                ...
                workspace_controller(&ws.read(cx), cx)
            })
            .and_then(|panel| {
    ...
    fn active_draft_text(&self, cx: &App) -> Option<SharedString> {
        ...
        let panel = workspace_controller(&workspace.read(cx), cx)?;
    ...
    fn rebuild_contents(&mut self, cx: &App) {
        ...
        // Re-derive ai_surface_visible from the active workspace so it stays
        // correct after workspace switches.
        self.ai_surface_visible = active_workspace
            .as_ref()
            .map_or(false, |ws| AiWorkspace::is_visible(ws, cx));

        // Derive active_thread_is_draft before focused_thread so draft
        // activations do not overwrite the eagerly updated focus state.
        self.active_thread_is_draft = active_workspace
            .as_ref()
            .and_then(|ws| workspace_controller(&ws.read(cx), cx))
            .map_or(false, |panel| panel.read(cx).active_thread_is_draft(cx));

        // Derive focused_thread from the active workspace's AI surface.
        // Only update when the controller gives us a positive signal — if the
        // controller returns None (e.g. still loading after a thread activation),
        ...
        for (ws_index, workspace) in workspaces.iter().enumerate() {
            ...
            let mut live_infos = all_thread_infos_for_workspace(workspace, cx);
    ...
    fn dispatch_context(&self, window: &Window, cx: &Context<Self>) -> KeyContext {
        ...
        dispatch_context.add(if self.filter_editor.focus_handle(cx).is_focused(window) {
            "searching"
        } else {
            "not_searching"
        });
    ...
    fn activate_thread_locally(
        ...
        // immediately, rather than waiting for a deferred AI surface
    ...
    fn stop_thread(&mut self, session_id: &acp::SessionId, cx: &mut Context<Self>) {
        ...
        let workspaces = multi_workspace.read(cx).workspaces().to_vec();
        for workspace in workspaces {
            if let Some(ai_workspace) = workspace_controller(&workspace.read(cx), cx) {
                let cancelled =
                    ai_workspace.update(cx, |panel, cx| panel.cancel_thread(session_id, cx));
                if cancelled {
                    return;
                }
            }
    ...
    fn archive_thread(
        ...
        if self.focused_thread.as_ref() == Some(session_id) {
            ...
            if let Some(next) = next_thread {
                ...
                if let Some(workspace) = &group_workspace {
                    workspace.update(cx, |workspace, cx| {
                        if let Some(ai_workspace) = workspace_controller(workspace, cx) {
                            ai_workspace.update(cx, |panel, cx| {
    ...
    fn render_thread(
        ...
        let is_selected = self.ai_surface_visible
    ...
    fn render_new_thread(
        ...
        let is_active = self.ai_surface_visible
...
mod tests {
    ...
    async fn test_new_thread_button_works_after_adding_folder(cx: &mut TestAppContext) {
        let project = init_test_project_with_ai_workspace("/project-a", cx).await;
    ...
    async fn test_absorbed_worktree_running_thread_shows_live_status(cx: &mut TestAppContext) {
        ...
        // thread inside it.
        let worktree_panel = add_ai_workspace(&worktree_workspace, &worktree_project, cx);
    ...
    async fn test_activate_archived_thread_reuses_workspace_in_another_window_with_target_sidebar(
        ...
        let _panel_b = add_ai_workspace(&workspace_b, &project_b, cx_b);
