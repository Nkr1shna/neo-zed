...
impl StatusBar {
        ...
        T: 'static + StatusItemView,
    {
        ...
        } else if position < self.left_items.len() + self.center_items.len() {
            self.center_items
    ...
    pub fn remove_item_at(&mut self, position: usize, cx: &mut Context<Self>) {
        if position < self.left_items.len() {
            ...
            self.center_items.remove(position - self.left_items.len());
        } else {
            self.right_items
                .remove(position - self.left_items.len() - self.center_items.len());
        }
        cx.notify();
    }
