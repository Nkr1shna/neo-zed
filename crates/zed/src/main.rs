...
fn files_not_created_on_launch(errors: HashMap<io::ErrorKind, Vec<&Path>>) {
    let message = "NeoZed failed to launch";
    ...
    #[cfg(any(target_os = "linux", target_os = "freebsd"))]
    {
        ...
        _cx.spawn(async move |_cx| {
            ...
            proxy
                .add_notification(
                    notification_id,
                    Notification::new("NeoZed failed to launch")
                        .body(Some(
                            format!(
...
fn main() {
    ...
    if args.system_specs {
        ...
        println!("NeoZed System Specs (from CLI):\n{}", system_specs);
        return;
    }
    ...
    let fs: Arc<dyn Fs> = Arc::new(RealFs::new(git_binary_path, app.background_executor()));
...
struct Args {
    ...
    /// On macOS, the default is `~/Library/Application Support/NeoZed`.
    /// On Linux/FreeBSD, the default is `$XDG_DATA_HOME/neozed`.
    /// On Windows, the default is `%LOCALAPPDATA%\NeoZed`.
