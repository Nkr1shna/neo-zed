...
fn main() {
    ...
    app.run(move |cx| {
        ...
        plugin_host::init(cx);
        plugin_store::init(app_state.client.clone(), cx);
...
struct Args {
    ...
    ///
    /// URLs can either be `file://` or `neozed://`, or relative to <https://neozed.dev>.
...
mod tests {
    use super::{
        FAIL_TO_OPEN_WINDOW_MESSAGE_PREFIX, LINUX_TROUBLESHOOTING_URL, STARTUP_FAILURE_MESSAGE,
    };
