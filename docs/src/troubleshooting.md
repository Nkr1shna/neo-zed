...
description: "Common issues and solutions for Neo Zed on all platforms."
---

# Troubleshooting

This guide covers common troubleshooting techniques for Neo Zed.
...
## Retrieve Neo Zed and System Information

When reporting issues or seeking help, it's useful to know your Neo Zed
version and system specifications. You can retrieve this information using the
following actions from the command palette:

- {#action zed::About}: Find your Neo Zed version number
- {#action zed::CopySystemSpecsIntoClipboard}: Populate your clipboard with Neo
  Zed version number, operating system version, and hardware specs

## Neo Zed Log

Often, a good first place to look when troubleshooting any issue in Neo Zed is
the Neo Zed log, which might contain clues about what's going wrong.
...
You'll find the Neo Zed log in the respective location on each operating
system:

- macOS: `~/Library/Logs/Neo Zed/Neo Zed.log`
- Windows: `C:\Users\YOU\AppData\Local\Neo Zed\logs\Neo Zed.log`
- Linux: `~/.local/share/neozed/logs/Neo Zed.log` or `$XDG_DATA_HOME`

> **Note:** In some cases, it might be useful to monitor the log live, such as
> when [developing a Neo Zed extension](https://neozed.dev/docs/extensions/developing-extensions).
> Example: `tail -f ~/Library/Logs/Neo\ Zed/Neo\ Zed.log`

The log may contain enough context to help you debug the issue yourself, or you
may find specific errors that are useful when filing a [GitHub issue](https://github.com/Nkr1shna/neo-zed/issues/new/choose)
or when talking to the Neo Zed community in our
[forums and discussions](https://neozed.dev/community-links#forums-and-discussions).

## Performance Issues (Profiling)

If you're running into performance issues in Neo Zed (hitches, hangs, or general
unresponsiveness), having a performance profile attached to your issue will help
us zero in on what is getting stuck.
...
1. With Neo Zed running, open Instruments
...
1. In the `Time Profiler` configuration, set the target to the running Neo Zed
   process
1. Start recording
...
1. Perform the action in Neo Zed that causes performance issues
1. Stop recording
...
Neo Zed creates local SQLite databases to persist data relating to its
workspace and your projects. These databases store, for instance, the tabs and
panes you have open in a project, the scroll position of each open file, the
list of all projects you've opened (for the recent projects modal picker), etc.
You can find and explore these databases in the following locations:

- macOS: `~/Library/Application Support/Neo Zed/db`
- Linux and FreeBSD: `~/.local/share/neozed/db` (or within `XDG_DATA_HOME` or
  `FLATPAK_XDG_DATA_HOME`)
- Windows: `%LOCALAPPDATA%\Neo Zed\db`

The naming convention of these databases takes on the form of `0-<channel>`:
...
which prevented Neo Zed from starting.
If you're experiencing startup issues, you can test whether it's
workspace-related by temporarily moving the database from its location, then
trying to start Neo Zed again.

> **Note**: Moving the workspace database will cause Neo Zed to create a fresh
> one.
