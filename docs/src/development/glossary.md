...
- `Panel`: An `Entity` implementing the `Panel` trait. Panels can be placed in a `Dock`. In the image below: `ProjectPanel` is in the left dock and `DebugPanel` is in the bottom dock. `Editor` does not implement `Panel`.
