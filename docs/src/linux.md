---
title: Neo Zed on Linux
description: "The installation script on the download page is the fastest way to install Neo Zed:"
---
...
The installation script on the [download](https://neozed.dev/download) page is the fastest way to install Neo Zed:

```sh
curl -f https://neozed.dev/install.sh | sh
```

We also offer a preview build of Neo Zed which receives updates about a week ahead of stable. You can install it with:

```sh
curl -f https://neozed.dev/install.sh | ZED_CHANNEL=preview sh
```

The Neo Zed installed by the script works best on systems that:
...
Both Nix and Alpine have third-party Neo Zed packages available (though they are currently a few weeks out of date). If you'd like to use our builds they do work if you install a glibc compatibility layer. On NixOS you can try [nix-ld](https://github.com/Mic92/nix-ld), and on Alpine [gcompat](https://wiki.alpinelinux.org/wiki/Running_glibc_programs).
...
## Other ways to install Neo Zed on Linux

Neo Zed is open source, and [you can install from source](./development/linux.md).

### Installing via a package manager

There are several third-party packages for various Linux distributions and package managers, sometimes under `zed-editor`. You may be able to install Neo Zed using these packages:
...
See [Repology](https://repology.org/project/zed-editor/versions) for a list of upstream `zed-editor` packages in various repositories.

### Community

When installing a third-party package please be aware that it may not be completely up to date and may be slightly different from the Neo Zed builds we package (a common change is to rename the binary to `zedit` or `zeditor` to avoid conflicting with other packages).

We'd love your help making Neo Zed available for everyone. If Neo Zed is not yet available for your package manager, and you would like to fix that, we have some notes on [how to do it](./development/linux.md#notes-for-packaging-zed).

The packages in this section provide binary installs for Neo Zed but are not official packages within the associated distributions. These packages are maintained by community members and as such a higher level of caution should be taken when installing them.

#### Debian and Ubuntu

Neo Zed is available in [this community-maintained repository](https://debian.griffo.io/).
...
If you'd prefer, you can install Neo Zed by downloading our pre-built .tar.gz. This is the same artifact that our install script uses, but you can customize the location of your installation by modifying the instructions below:

Download the `.tar.gz` file:

- [neozed-linux-x86_64.tar.gz](https://cloud.neozed.dev/releases/stable/latest/download?asset=neozed&arch=x86_64&os=linux&source=docs)
  ([preview](https://cloud.neozed.dev/releases/preview/latest/download?asset=neozed&arch=x86_64&os=linux&source=docs))
- [neozed-linux-aarch64.tar.gz](https://cloud.neozed.dev/releases/stable/latest/download?asset=neozed&arch=aarch64&os=linux&source=docs)
  ([preview](https://cloud.neozed.dev/releases/preview/latest/download?asset=neozed&arch=aarch64&os=linux&source=docs))
...
If Neo Zed was installed using a package manager, please consult the documentation for that package manager on how to uninstall a package.

## Troubleshooting

Linux works on a large variety of systems configured in many different ways. We primarily test Neo Zed on a vanilla Ubuntu setup, as it is the most common distribution our users use, that said we do expect it to work on a wide variety of machines.

### Neo Zed fails to start

If you see an error like "/lib64/libc.so.6: version 'GLIBC_2.29' not found" it means that your distribution's version of glibc is too old. You can either upgrade your system, or [install Neo Zed from source](./development/linux.md).
...
You can find out which graphics card Neo Zed is using by looking in the Neo Zed log (`~/.local/share/neozed/logs/Neo Zed.log`) for `Using GPU: ...`.
...
On others, you may be able to use the environment variable `DRI_PRIME=1` when running Neo Zed to force the use of the discrete GPU.
...
#### Forcing Neo Zed to use a specific GPU

There are a few different ways to force Neo Zed to use a specific GPU:

##### Option A

You can use the `ZED_DEVICE_ID={device_id}` environment variable to specify the device ID of the GPU you wish to have Neo Zed use.
...
where the device ID here is `2484`. This value is in hexadecimal, so to force Neo Zed to use this specific GPU you would set the environment variable like so:

```
ZED_DEVICE_ID=0x2484 neozed
```
...
If you are using Mesa, you can run `MESA_VK_DEVICE_SELECT=list neozed --foreground` to get a list of available GPUs and then export `MESA_VK_DEVICE_SELECT=xxxx:yyyy` to choose a specific device. Furthermore, you can fallback to xwayland with an additional export of `WAYLAND_DISPLAY=""`.
...
If Vulkan is configured correctly, and Neo Zed is still not working for you, please [file an issue](https://github.com/zed-industries/zed) with as much information as possible.

When reporting issues where Neo Zed fails to start due to graphics initialization errors on GitHub, it can be impossible to run the `zed: copy system specs into clipboard` command like we instruct you to in our issue template. We provide an alternative way to collect the system specs specifically for this situation.

Passing the `--system-specs` flag to the Neo Zed binary like
...
Additionally, it is extremely beneficial to provide the contents of your Neo Zed log when reporting such issues. The log is usually located at `~/.local/share/neozed/logs/Neo Zed.log`. The recommended process for producing a helpful log file is as follows:

```sh
truncate -s 0 ~/.local/share/neozed/logs/Neo\ Zed.log # Clear the log file
ZED_LOG=wgpu=info neozed .
cat ~/.local/share/neozed/logs/Neo\ Zed.log
# copy the output
```

Or, if you have the Neo Zed CLI set up, you can do

```sh
ZED_LOG=wgpu=info /path/to/neozed --foreground .
# copy the output
```
...
````
<details><summary>Neo Zed Log</summary>

```
{Neo Zed log contents}
```
...
### Neo Zed isn't remembering my API keys

### Neo Zed isn't remembering my login
...
Neo Zed needs a place to securely store secrets such as your Neo Zed login cookie or your OpenAI API keys and we use a system-provided keychain to do this. Examples of packages that provide this are `gnome-keyring`, `KWallet` and `keepassxc` among others.

### Could not start inotify

Neo Zed relies on inotify to watch your filesystem for changes. If you cannot start inotify then Neo Zed will not work reliably.

If you are seeing "too many open files" then first try `sysctl fs.inotify`.

- You should see that max_user_instances is 128 or higher (you can change the limit with `sudo sysctl fs.inotify.max_user_instances=1024`). Neo Zed needs only 1 inotify instance.
- You should see that `max_user_watches` is 8000 or higher (you can change the limit with `sudo sysctl fs.inotify.max_user_watches=64000`). Neo Zed needs one watch per directory in all your open projects + one per git repository + a handful more for settings, themes, keymaps, extensions.

It is also possible that you are running out of file descriptors. You can check the limits with `ulimit` and update them by editing `/etc/security/limits.conf`.

### No sound or wrong output device

If you're not hearing any sound in Neo Zed or the audio is routed to the wrong device, it could be due to a mismatch between audio systems. Neo Zed relies on ALSA, while your system may be using PipeWire or PulseAudio. To resolve this, you need to configure ALSA to route audio through PipeWire/PulseAudio.
...
On X11 systems, Neo Zed automatically detects the appropriate scale factor for high-DPI displays. The scale factor is determined using the following priority order:
...
If you want to customize the scale factor beyond what Neo Zed detects automatically, you have several options:
...
If this command returns no output, Neo Zed is using RandR (X11's monitor management extension) to automatically calculate the scale factor based on your monitor's reported resolution and physical dimensions.

#### Option 1: Set Xft.dpi (X Resources Database)

`Xft.dpi` is a standard X11 setting that many applications use for consistent font and UI scaling. Setting this ensures Neo Zed scales the same way as other X11 applications that respect this setting.
...
Restart Neo Zed for the changes to take effect.

#### Option 2: Use the GPUI_X11_SCALE_FACTOR environment variable

This Neo Zed-specific environment variable directly sets the scale factor, bypassing all automatic detection.

```sh
GPUI_X11_SCALE_FACTOR=1.5 neozed
```
...
Replace `192` with your desired DPI value. This affects the system globally and will be used by Neo Zed's automatic RandR detection when `Xft.dpi` is not set.

### Font rendering parameters

On Linux, Neo Zed reads `ZED_FONTS_GAMMA` and `ZED_FONTS_GRAYSCALE_ENHANCED_CONTRAST` environment variables for the values to use for font rendering.
