...
- **Open source.** The editor and all AI features are [open source](https://github.com/Nkr1shna/neo-zed). You can read how AI is implemented, how data flows to providers, and how tool calls execute.
...
- **External agents.** Run Claude Agent, Gemini CLI, Codex, and other CLI-based agents directly in Zed through the [Agent Client Protocol](https://neozed.dev/acp). See [External Agents](./external-agents.md).
...
New to Zed? Start with [Getting Started](../getting-started.md), then come back here to set up AI. For a higher-level overview, see [neozed.dev/ai](https://neozed.dev/ai).
